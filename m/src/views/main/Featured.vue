<template>
  <div class="blog">
    <!--头部-->
    <Header :author="author" :subContent="subContent" :headerBG="headerBG" title="Featured"></Header>
    <!--内容部分-->
    <!--内容部分-->
    <template v-for="(content,index) in contents" v-key="index">
      <Content :content="content" :class="{'post-preview':true, animated:true, bounceInLeft:animate,bounceInDown:!animate}" fromFeature="1"></Content>
    </template>
    <!-- 返回顶部 -->
...
export default {
  ...
  methods: {
    ...
    dealSelect(page){
      ...
      this.$http
        .get("http://localhost:8088/featured/select",{
          params:{
            page:page
          }
        })
